...
- Bucket contract: canonical KG tables live in `gs://jouvencekb/main`, native inputs in `raw/`, immutable derived neighbor-sampling/PyG snapshots in `pyg/<graph-build-id>/`, Lamin internals in private `.lamin/`, and short-lived candidates only in `staging/`. `pyg/` is reproducible training infrastructure, not canonical biology. A prefix-scoped lifecycle deletes `staging/**` after 14 days; it does not apply to `raw/`, `main/`, `pyg/`, or `.lamin/`.
